...
useSWR 中请求是相对于 key 的，但 fetcher 和配置却是相对于 Hook 的，比如同 key 的 useSWR 是可以使用不同的 fetcher 和配置的。尽管我们不会那样写，但还是会造成理解负担。关于这点我们可通过[最佳实践-代码组织]()来避免，保证相同 key 的请求的 fetcher 和 config 是一致的。
